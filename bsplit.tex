...
So, I have fixed the problem. What was happening was that the initial field I was working
with was unnormalized. 

If I define the field to be $\tilde{\phi}(t)=\sqrt{\frac{2}{\kappa}}g(t)e^{\tau}$, then

\begin{equation}
\int^\infty dt |\tilde{\phi}(t)=\sqrt{\frac{2}{\kappa}}g(t)e^{\tau}|^2 =1
\end{equation}

But we can see from evaluating this integral that:
\begin{align}
\int^\infty dt |\tilde{\phi}(t)=\sqrt{\frac{2}{\kappa}}g(t)e^{\tau}|^2 &=1
\int^infty dt |\tilde{\phi}(t)=\sqrt{\frac{2}{\kappa}}g(t)e^{\tau}|^2 &=e^{2\tau}-1 i\neq 1\end{align}

So, I actually needed to define the field as 

\begin{equation}
\tilde{\phi}(t)=\frac{\sqrt{\frac{2}{\kappa}}g(t)e^{\tau}}{e^{2\tau(t_\infty)}-1}
\end{equation}

This ensures that the wave-packet sums to one over all time. When I use this
condition, I get the following graph:
\begin{figure}
...
I will include the normalization in the derivation in the next draft of these notes.
